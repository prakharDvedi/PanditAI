        ...
        for rule in rules:
            ...
            session.run("""
                ...
                SET r.text = $prediction, 
                    ...
                    r.source = $source,
                    r.type = $type
            """, id=rule["id"], prediction=rule["prediction"], 
                 condition=rule["condition"], source=rule["source"], type=rule["type"])
            ...
            for entity in rule.get("entities", []):
                ...
                if "Sun" in entity: e_name = "Sun"
                elif "Moon" in entity: e_name = "Moon"
                elif "Jupiter" in entity: e_name = "Jupiter"
                elif "Mars" in entity: e_name = "Mars"
                elif "Saturn" in entity: e_name = "Saturn"
                elif "Venus" in entity: e_name = "Venus"
                elif "Mercury" in entity: e_name = "Mercury"
                else: e_name = None
                ...
                if e_name:
                    ...
                    """, id=rule["id"], p_name=e_name)

            # 3. Handle House Numbers (if mentioned in condition)
            # Simple regex to find "10th House" or "10"
            import re
            house_match = re.search(r'(\d+)(st|nd|rd|th)?\s+House', rule["condition"], re.IGNORECASE)
            if house_match:
